...
var updaterVersion;
var configoptions = ["showPanel", "showConsole", "showButtons", "version"];
var configuration = {
  showPanel: true,
  showConsole: true,
  showButtons: true,
...
function checkForUpdates() {
  ...
  if (updateAvaliable) {
    log(
      '</a><a style="color:red">Update avaliable: ' +
        updaterVersion.version +
        '</a><a href="' +
        document.location.protocol +
        '//github.com/Willsr71/MoodleUtils/raw/master/moodleutils.user.js">[link]'
    );
...
function createUpdater() {
  ...
  updater.src =
    document.location.protocol +
    "//willsr71.net/scripts/version.php?script=moodleutils&lang=javascript";
...
function checkLoaded() {
  if (
    !shouldLoad ||
    databaselist.length - failedDatabaseCount == Object.size(databases)
  ) {
    log("Loaded " + getData().length + " terms from localstorage");
    log(
      "Loaded " +
        appendDatabases(getData()).length +
        " terms from " +
        (Object.size(databases) + 1) +
        " databases"
    );
    log("Done.");
  } else {
...
function addDatabase(name) {
  ...
  script.src =
    document.location.protocol +
    "//willsr71.net/scripts/moodle/databases/" +
    name +
    ".js";
...
function getData() {
  var data = JSON.parse(
    window.localStorage.getItem("moodleutils_local_database")
  );
...
function autoFill() {
  ...
  while (true) {
    ...
    if (q.classList.contains("shortanswer")) {
      ...
      question = contentnode.getElementsByClassName("qtext")[0].innerHTML;
      answernode = contentnode.childNodes[0].getElementsByClassName(
        "answer"
      )[0];
      ...
      questionnode = contentnode.childNodes[0].getElementsByClassName(
        "answer"
      )[0];
...
function autoRecord() {
  ...
  while (true) {
    ...
    if (q.classList.contains("shortanswer")) {
      question = contentnode.childNodes[0].getElementsByClassName("qtext")[0]
        .innerHTML;
      answer = contentnode.childNodes[1]
        .getElementsByClassName("rightanswer")[0]
        .innerHTML.replace("The correct answer is: ", "");
      ...
      var correcttext = contentnode.childNodes[1]
        .getElementsByClassName("rightanswer")[0]
        .innerHTML.replace("The correct answer is: ", "");
      var questionnode = contentnode.childNodes[0].getElementsByClassName(
        "answer"
      )[0];
      var questions = [];
...
function autoClear() {
  var data = getData();
  var confirmmsg =
    "Are you sure you want to clear " + data.length + " answers?";
